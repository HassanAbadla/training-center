...
<div class="flex flex-row min-h-screen h-auto">
    ...
    <div class="px-8 py-8 flex justify-center w-full" >
        <div class="w-2/3 p-4 border border-gray-200 rounded-xl bg-gray-50">
            ...
            <div class="">
                <h1 class="mb-6 pb-2 font-bold text-xl border-b-2 border-yellow-500 leading-loose">بيانات الموظف</h1>
            </div>
                
            <div class="flex justify-center">
                
            <form action="/updateemp/{{ $employees->id }}" method="POST" class="pt-6 px-4 bg-gray-50 flex flex-col jutify-center w-2/3 ">
                @csrf
                {{ method_field('PUT') }}
                    <label for="empName" class="py-2 px-2"> اسم الموظف
                        <input type="text" name="empName" class="h-8 w-3/5 float-left  md:mr-6 md:h-8 w-3/4  rounded-lg px-2 focus:outline-none ring-1 ring-yellow-200 focus:ring-2 focus:ring-yellow-400" value="{{ $employees->Emp_name }}">
                    </label>
                    <label for="empMobile" class="py-2 px-2"> رقم الجوال
                        <input type="number" name="empMobile" class="h-8 w-3/5 float-left md:mr-6 md:h-8 w-3/4  rounded-lg px-2 focus:outline-none ring-1 ring-yellow-200 focus:ring-2 focus:ring-yellow-400" value="{{ $employees->Emp_mobile }}">
                    </label>
                    ...
                    <select type="number" name="empBranch" class="h-8 w-3/5 float-left md:mr-6 md:h-8 w-3/4  rounded-lg px-2 focus:outline-none ring-1 ring-yellow-200 focus:ring-2 focus:ring-yellow-400">
                        <option value="{{ $employees->Emp_branch }}">{{ $employees->Emp_branch }}</option>
                    ...
                    <label for="speciality" class="py-2 px-2">التخصص
                        <input type="text" name="speciality" class="h-8 w-3/5 float-left md:mr-6 md:h-8 w-3/4  rounded-lg px-2 focus:outline-none ring-1 ring-yellow-200 focus:ring-2 focus:ring-yellow-400" value="{{ $employees->speciality }}">
                    </label>
                    <label for="job" class="py-2 px-2">الوظيفة
                        <input type="text" name="job" class="h-8 w-3/5 float-left md:mr-6 md:h-8 w-3/4  rounded-lg px-2 focus:outline-none ring-1 ring-yellow-200 focus:ring-2 focus:ring-yellow-400" value="{{ $employees->job }}">
                    </label>
                    ...
                    <form class="w-full pt-8 " action="/deleteemp/{{ $employees->id }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button class="btn border-2 bg-gray-100 border-red-500 h-12 w-full md:w-full rounded-xl text-red-900 hover:bg-red-500 hover:text-white transition ease-out duration-500 ">حذف </button>
                    </form>
                    </div>
                </div>
            <!-- </div> -->
